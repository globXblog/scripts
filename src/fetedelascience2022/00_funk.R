...
getData <- function(dataset,normalizeAnom=FALSE,
                    seasons=rbind(c(3,4,5),c(6,7,8),c(9,10,11),c(12,1,2))){
  if(any(names(dataset)=='iseason')){ # Not used - earlier attempt with another dataset
    seasonnal=cbind(dataset,anom=NA)
    for(i in 1:max(seasonnal$iseason)){
      mask=seasonnal$iseason==i
      foo=seasonnal$value[mask]
      if(normalizeAnom){
        anom=qnorm((rank(foo,na.last='keep')-0.5)/NROW(foo))
      } else {
        anom=scale(foo)
      }
      seasonnal$anom[mask]=anom
    }
  } else {
    seasonnal=c()
...
createSound <- function(seasonnal,ID,fname,normalizeAnom=FALSE,style='major',bpm=90,
                        seasons=levels(seasonnal$season),pow=2,t0=0){
...
createTheme <- function(bpm=90,rep=4,t1=0,t2=Inf,t3=Inf,t4=Inf,trimTo=Inf){
  spb=60/bpm # seconds per beat
  spt=spb/4 # seconds per time step (here time step = 16th note i.e. 1/4 of a beat)
  # Harmonics
  tim=((1:(rep*16))-1)*spt*2+0.01
  nt=length(tim)
  n0=rep_len(c('A3','E4','A4','B4','E4','A3','A3','B3',
               'B3','E3','E3','B3','E4','A4','B4','E4'),nt)
  v0=rep_len(c(1,0.7,0.6,0.65,0.1,0.8,0.0,1,
               0.0,0.8,0,0.6,0.1,0.4,0.5,0.4),nt)
  pan=rep_len(0.8*c(0,-1,1,-1,0,1,0,-1,
                    0,0,0,1,0,-1,1,-1),nt)
  harm=play.instrument(inst$guitar,notes=n0,time=tim,volume=v0,pan=pan,fadeout=rep(Inf,nt))
  # Bass
  n0=rep_len(c('A1','A1','A1','A1','A1','A3','A2','B2',
               'B3','E3','E3','E3','E2','E2','E2','E2'),nt)
  v0=rep_len(c(1,0,0,0,0,0.5,0.7,1,0,1,0,0,1,0,0,0),nt)
  bass=play.instrument(inst$bass,notes=n0,time=tim,volume=v0,fadeout=rep(Inf,nt))
  # Mix and fade in/out
  final=mix(waves=c(list(bass,harm)),vol=c(0.4,1))
  d=length(final@left)/final@samp.rate
  f1=min(t1/d,1);f2=min(t2/d,1);f3=min(t3/d,1);f4=min(t4/d,1)
  env=envelope(t=c(0,f1,f2,f3,f4,1),v=c(0,1,1,0.2,0,0))
  left=applyEnvelope(as.soundSample(final,-1),env)
  right=applyEnvelope(as.soundSample(final,1),env)
  lastn=min(trimTo*left$rate,left$n)
  wleft=left$wave[1:lastn]
  wright=right$wave[1:lastn]
  w <- tuneR::Wave(left=wleft,right=wright,samp.rate=final@samp.rate,bit=16)
...
createAudioLegend <- function(bpm=90,style='major',n=200,rmin=5,rmax=30,pow=10){ 
  # Not used - earlier attempt at creating audio legends for scales 
  spb=60/bpm # seconds per beat
  spt=spb/4 # seconds per time step (here time step = 16th note i.e. 1/4 of a beat)
  if(style=='oriental'){
    n1=c('E3','G3','A3','B3','C4','Eb4','E4','Gb4','G4','A4','B4','C5')
  ...
  # Time
  rate=rescale((1:n)*(n:1),rmin,rmax)
  tim=cumsum(rexp(n,rate))
  ix=sample(n1,n,replace=TRUE,prob=1:length(n1))
  vol=rescale(runif(n)^pow)
  pan=runif(n,-1,1)
  i1=play.instrument(inst$piano,notes=ix,time=tim,volume=vol,pan=pan,fadeout=Inf+0*tim)
  tim=cumsum(rexp(n,rate))
  ix=sample(n2,n,replace=TRUE,prob=1:length(n2))
  vol=rescale(runif(n)^pow)
  pan=runif(n,-1,1)
  i2=play.instrument(inst$hangDrum,notes=ix,time=tim,volume=vol,pan=pan,fadeout=Inf+0*tim)
